 ...
 *
 * - **`PRIVATE_GALLERY_STORE`** — `off` (the default) or `enabled`. The feature
 *   is post-MVP and un-provisioned on every deployment today, so `off` is the
 *   safe default and an operator opts in explicitly. Unlike `SITE_CONTENT_SOURCE`
 *   there is no "a default silently serves the wrong thing" hazard: `off` serves
 *   nothing.
...
export type PrivateGalleryStoreMode = "off" | "enabled";
...
const STORE_MODES: readonly PrivateGalleryStoreMode[] = ["off", "enabled"];
...
function parseStoreMode(value: string | undefined): PrivateGalleryStoreMode {
  ...
  return trimmed as PrivateGalleryStoreMode;
}
...
export function readPrivateGalleryDeployment(
  ...
  return {
    store: parseStoreMode(environment[settingNames.store]),
...
export function getPrivateGalleryDeployment(): PrivateGalleryDeployment {
  cached ??= readPrivateGalleryDeployment(process.env);
  return cached;
}
