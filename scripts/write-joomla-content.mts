...
class JoomlaWriteError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "JoomlaWriteError";
  }
}

function fail(message: string): never {
  throw new JoomlaWriteError(message);
}
...
if (import.meta.main || process.argv[1] === fileURLToPath(import.meta.url)) {
  main().catch((cause) => {
    console.error(`Joomla write failed: ${cause instanceof Error ? cause.message : String(cause)}`);
    process.exitCode = 1;
  });
}
